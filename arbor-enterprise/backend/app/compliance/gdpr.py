...
class GDPRDeletionJob:
    ...
    async def execute(self, user_id: str) -> DeletionResult:
        ...
        try:
            ...
            for table_name, id_column in self.PII_TABLES[1:]:  # Skip users table
                try:
                    count = await self._hard_delete_table(table_name, id_column, user_id)
                    if count > 0:
            ...
            for table_name, id_column in self.ANONYMIZE_TABLES:
                try:
                    count = await self._anonymize_table(table_name, id_column, user_id)
                    if count > 0:
    ...
    async def _anonymize_table(
        ...
        stmt = text(f"UPDATE {table_name} SET {set_clause} " f"WHERE {id_column} = :user_id")
    ...
    async def _log_deletion_start(self, user_id: str) -> None:
        ...
        from app.compliance.audit_log import AuditAction, AuditLogger
