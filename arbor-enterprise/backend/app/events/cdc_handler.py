...
class CDCListener:
    ...
    async def setup(self) -> None:
        ...
        try:
            ...
            await conn.execute(f"""
                ...
                BEGIN
                    IF NOT EXISTS (
                        SELECT 1 FROM pg_publication WHERE pubname = '{self.publication_name}'
                    ) THEN
                        CREATE PUBLICATION {self.publication_name}
                        FOR TABLE arbor_entity, arbor_enrichment, arbor_curation;
                    END IF;
                END
                $$;
            """)
    ...
    async def start(self) -> None:
        ...
        self._connection = await asyncpg.connect(
            ...
            server_settings={"application_name": "arbor_cdc"}
        )
        ...
        while self._running:
            try:
                # Poll for changes
                changes = await self._connection.fetch(f"""
